...
const IMPUESTO = 0.0496
const AYUDANTE = 130
var ayudantes
var resumen_ayudante
var accesorios
var resumen_accesorios
...
$(document).on('turbolinks:load', function () {
  ...
  $('#vehicle_select').on('change', function(evt){
    ...
    data = $.get(`vehicles/${vehicle_id}.json`, function (data) {
      $('#vehicleInfo').html('')
      ejes = data.edges
      pago_por_km = parseFloat(data.payment)
  ...
  $('#route_select').on('change', function(evt){
    ...
    data = $.get(`routes/${route_id}.json`, function (data) {
      ...
      litros_necesarios = (distancia * 2 )/ rendimiento_por_km
      if( ejes >= 2 && ejes <= 4){
  ...
  $('#calcButton').on('click', function(evt){
    evt.preventDefault();
    dias = $('#days').val()
    ayudantes = $('#assistants').val()
    accesorios = $ ('#cost_of_accessories').val()
    ...
    resumen_combustible_final=resumen_combustible + resumen_combustible * IMPUESTO
    resumen_comida = dias * COMIDA
    resumen_ayudante = ayudantes * AYUDANTE
    resumen_accesorios = accesorios * 1
    resumen_operacion = pago_conduccion + casetas + resumen_comida + resumen_combustible_final + resumen_ayudante + resumen_accesorios
    console.log(resumen_operacion)
    $('#resumeFood').html( '$' + resumen_comida.toFixed(2) )
    $('#resumeAssistants').html( '$' + resumen_ayudante.toFixed(2) )
    $('#resumeFuel').html( '$' + resumen_combustible.toFixed(2) )
    $('#resumeFinalFuel').html( '$' + resumen_combustible_final.toFixed(2) )
    $('#resumeCostAccesories').html( '$' + resumen_accesorios.toFixed(2) )
